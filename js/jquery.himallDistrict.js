	...
	$.fn.himallDistrict = function(options) {
		...
		this.each(function() {
			...
			for(var i=0; i<params.items.length;i++){
				provinceStr+='<li><a data-id="'+params.items[i].v+'" data-index="'+i+'">'+params.items[i].t+'</a></li>';
			}
			provinceStr+='</ul>';
			...
			params.renderTo.on('click','.province-ul a',function() {
				...
				for(var i=0; i<parent.s.length;i++){
					cityStr+='<li><a data-id="'+parent.s[i].v+'" data-index="'+i+'">'+parent.s[i].t+'</a></li>';
				}
			...
			params.renderTo.on('click','.city-ul a',function() {
				var countyStr='';
				var parent=params.items[provinceIndex].s[$(this).data('index')];
				cityId=parent.v;
				$(this).parent().addClass('cur').siblings().removeClass();
				if(parent.s!=null){
					...
					for(var i=0; i<parent.s.length;i++){
						countyStr+='<li><a data-id="'+parent.s[i].v+'">'+parent.s[i].t+'</a></li>';
					}
